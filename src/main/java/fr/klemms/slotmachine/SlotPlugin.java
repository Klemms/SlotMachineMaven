...
public class SlotPlugin extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (Util.canUseDialogs()) {
			...
			dialogHandlers.add(DialogInputNumber.instance);
		}

		if (Bukkit.getPluginManager().isPluginEnabled("AdvancedEnchantments")) {
			this.getLogger().log(Level.SEVERE, ChatContent.RED + SlotPlugin.CHAT_PREFIX + "WARNING ! You seem to have installed " + ChatContent.GOLD + "[AdvancedEnchantments]" + ChatContent.RED + ". This plugin interferes with Anvil GUIs generated by other plugins. Functionality may not work.");
		}
	...
	public static void saveCooldownsToDisk() {
		if (SlotPlugin.playerConfigs != null) {
			PlayerConfig.writePlayerConfig();
		}
	}
